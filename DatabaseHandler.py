...
class DatabaseHandler:
    ...
    def save_speed_test_results(self, session_id, download, upload, ping, latitude, longitude):
        try:
            # Save the speed test results using Django ORM
            internet = Internet(
                download=download,
                upload=upload,
                ping=ping,
            ...
            internet.save()
            print(f"Saved speed test results: {download} Mbps / {upload} Mbps / {ping} ms (ID: {unique_id})")
        except Exception as e:
            print(f"Error saving speed test results: {e}")
            raise
