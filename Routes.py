...
class Routes:
    ...
    def setup_routes(self):
        ...
        def save_speed_test_results():
            ...
            if download is not None and upload is not None and ping is not None and session_id is not None:
                with self.session_lock:
                    self.user_sessions[session_id] = (float(download), float(upload), float(ping))
                return jsonify({"status": "Speedtest Data Saved", "session_id": session_id}), 200
            return jsonify({"error": "Invalid data"}), 400
